...
@listener(outgoing=True, command="s",
...
@listener(outgoing=True, command="tql",
          description="收集回复的图像/贴纸作为贴纸，通过参数指定 emoji 以设置非默认的 emoji。",
          parameters="<emoji>")
async def sticker(context):
    """ Fetches images/stickers and add them to your pack. """
    ...
    if photo:
        ...
        pack_name = f"dalao_quotation_add"
        pack_title = f"@{user.username} 的私藏 ({pack})"
        command = '/newpack'
        file = BytesIO()
...
A pack can't have more than 120 stickers at the moment.":
                    pack += 1
                    pack_name = f"dalao_quotation"
                    pack_title = f"@{user.username} 的私藏 ({pack})"
                    await context.edit("偷贴纸已进行80%")
                    await conversation.send_message(pack_name)
                    chat_response = await conversation.get_response()
                    if chat_response.text == "Invalid pack selected.":
                        await add_sticker(conversation, command, pack_title, pack_name, animated, message,
                                          context, file, emoji)
                        await context.edit(
                            f"꧁♛꧂](t.me/addstickers/dalao_quotation_add) ",
                            parse_mode='md')
                        return
                await upload_sticker(animated, message, context, file, conversation)
                await conversation.get_response()
...
@listener(outgoing=True, command="ymd",
          description="收集回复的图像/贴纸作为贴纸，通过参数指定 emoji 以设置非默认的 emoji。",
          parameters="<emoji>")
async def sticker(context):
    """ Fetches images/stickers and add them to your pack. """
...
A pack can't have more than 120 stickers at the moment.":
                    ...
                    if chat_response.text == "Invalid pack selected.":
                        await add_sticker(conversation, command, pack_title, pack_name, animated, message,
                                          context, file, emoji)
                        await context.edit(
                            f"꧁♛꧂](t.me/addstickers/yamiedie_pic) ",
                            parse_mode='md')
                        return
                await upload_sticker(animated, message, context, file, conversation)
                await conversation.get_response()
